...
import static project_300.Token_process_business.conj;

/**
 *
 * @author doha
 */
public class Token_process_entertain {
    ...
    void process() throws IOException, InterruptedException {
        ...
        for(int i=0; i<tem_vec.size(); i++) {
            ...
            if( f==0 ) {
                p.entertain_word.add(tem_vec.get(i));
            }
        }
        
        File file = new File("entertain_news_words.txt");
        file.createNewFile();
        FileWriter output = new FileWriter(file);
        
        for(int i=0; i<p.entertain_word.size(); i++) {
            
            output.write(p.entertain_word.get(i));
            output.write("\r\n");
        }
        output.flush();
        output.close();
