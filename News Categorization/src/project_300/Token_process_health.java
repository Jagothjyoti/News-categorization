 ...
 * @author doha
 */
public class Token_process_health {
    ...
    void process() throws IOException, InterruptedException {
        ...
        for(int i=0; i<tem_vec.size(); i++) {
            ...
            if( f==0 ) {
                p.health_word.add(tem_vec.get(i));
            }
        }
        
        File file = new File("health_news_words.txt");
        file.createNewFile();
        FileWriter output = new FileWriter(file);
        
        for(int i=0; i<p.health_word.size(); i++) {
            
            output.write(p.health_word.get(i));
            output.write("\r\n");
        }
        output.flush();
        output.close();        
