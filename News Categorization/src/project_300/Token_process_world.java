 ...
 * @author doha
 */
public class Token_process_world {
    ...
    void process() throws IOException, InterruptedException {
        ...
        for(int i=0; i<tem_vec.size(); i++) {
            ...
            if( f==0 ) {
                p.world_word.add(tem_vec.get(i));
                //System.out.println(tem_vec.get(i));
            }
        }
        
        File file = new File("world_news_words.txt");
        file.createNewFile();
        FileWriter output = new FileWriter(file);
        
        for(int i=0; i<p.world_word.size(); i++) {
            
            output.write(p.world_word.get(i));
            output.write("\r\n");
        }
        output.flush();
        output.close(); 
