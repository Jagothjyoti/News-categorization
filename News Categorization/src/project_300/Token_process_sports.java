...
class Token_process_sports {
    ...
    void process() throws IOException, InterruptedException {
        ...
        for(int i=0; i<tem_vec.size(); i++) {
            ...
            if( f==0 ) {
                p.sports_word.add(tem_vec.get(i));
            }
        }
        
        File file = new File("sports_news_words.txt");
        file.createNewFile();
        FileWriter output = new FileWriter(file);
        
        for(int i=0; i<p.sports_word.size(); i++) {
            
            output.write(p.sports_word.get(i));
            output.write("\r\n");
        }
        output.flush();
        output.close();
